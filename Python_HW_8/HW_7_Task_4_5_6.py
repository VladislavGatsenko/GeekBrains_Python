...
mustek = Scanner("Mustek", "White", 150, "A3")
...
print(f"1) Состояние склада до перемещения техники:\n>>> {stock.items}\n")
stock.move(0, bookkeeping)
print(f"2) Позиция:\n>>> {bookkeeping.items}\nперемещена в отдел бухгалтерии\n")
print(f"3) Состояние склада после перемещения техники:\n>>> {stock.items}\n")

epson = Printer("Epson", "Blue", "one hundred", "A4", "Здесь должен быть INT")

try:
    print(epson.check_input())
except ValueExeption:
    print(f"4) Проверка не пройдена, для {epson.item_name} параметр 'price' должен быть числовым")
